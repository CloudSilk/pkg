...
func (sf *client) ReadWrite(slaveID byte, funcCode byte, data []byte) (results []byte, err error) {
	if slaveID < sf.addressMin || slaveID > sf.addressMax {
		return nil, fmt.Errorf("modbus: slaveID '%v' must be between '%v' and '%v'",
			slaveID, sf.addressMin, sf.addressMax)
	}
...
func (sf *client) ReadWriteWithSubFuncCode(slaveID byte, funcCode, subFuncCode byte, data []byte) (results []byte, err error) {
	if slaveID < sf.addressMin || slaveID > sf.addressMax {
		return nil, fmt.Errorf("modbus: slaveID '%v' must be between '%v' and '%v'",
			slaveID, sf.addressMin, sf.addressMax)
	}
	...
	response, err := sf.Send(slaveID, ProtocolDataUnit{
		FuncCode: funcCode,
		Data:     append([]byte{subFuncCode}, data...),
	})
