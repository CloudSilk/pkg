...
func (sf *protocolFrame) encodeTCPFrame(tid uint16, slaveID byte,
	pdu ProtocolDataUnit, crc CRCType, crcEndian bool) (protocolTCPHeader, []byte, error) {
	...
	if crc != CRCNone {
		checksum := CRC16(adu[6:tcpHeaderMbapSize+1+len(pdu.Data)], crc)
		startIndex := tcpHeaderMbapSize + 1 + len(pdu.Data)
		if crcEndian {
			adu[startIndex] = byte(checksum >> 8)
			adu[startIndex+1] = byte(checksum)
		} else {
			adu[startIndex] = byte(checksum)
			adu[startIndex+1] = byte(checksum >> 8)
		}
	}
	return head, adu, nil
}
...
func decodeTCPFrame(adu []byte, crcType CRCType, crcEndian bool) (protocolTCPHeader, []byte, error) {
	...
	if crcType != CRCNone {
		...
		crc := CRC16(adu[6:len(adu)-2], crcType)
		expect := binary.LittleEndian.Uint16(adu[len(adu)-2:])
		// fmt.Printf("adu length:%d,pdu:%d,received222 [%x],crc:[%x],expect:[% x],data:[%x]\n", len(adu), pduLength, adu[tcpHeaderMbapSize+1:len(adu)-2], crc, expect, adu[tcpHeaderMbapSize:len(adu)-2])
		if crc != expect {
			return head, nil, fmt.Errorf("modbus: response crc '%x' does not match expected '%x'", expect, crc)
		}
		return head, adu[tcpHeaderMbapSize : len(adu)-2], nil
	}
...
func (sf *TCPClientProvider) Send(slaveID byte, request ProtocolDataUnit) (ProtocolDataUnit, error) {
	...
	// sf.Debugf("sending111 [% x]", request.Data)
	head, aduRequest, err := frame.encodeTCPFrame(tid, slaveID, request, sf.crcType, sf.crcEndian)
	if err != nil {
		return response, err
	}
	...
	rspHead, pdu, err := decodeTCPFrame(aduResponse, sf.crcType, sf.crcEndian)
	if err != nil {
		return response, err
	}
...
func (sf *TCPClientProvider) SendPdu(slaveID byte, pduRequest []byte) ([]byte, error) {
	...
	rspHead, rspPdu, err := decodeTCPFrame(aduResponse, sf.crcType, sf.crcEndian)
	if err != nil {
		return nil, err
	}
...
func (sf *TCPClientProvider) setCRC(t CRCType, endian bool) {
	sf.crcType = t
	sf.crcEndian = endian
}
