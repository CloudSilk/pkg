		...
		for rdCnt, length := 0, tcpHeaderMbapSize; rdCnt < length; {
			...
			if err != nil {
				...
				if e, ok := err.(net.Error); ok && !e.Temporary() {
					fmt.Printf("e.Temporary() error:%v\n", err)
					return
				}
...
func (sf *ServerSession) frameHandler(requestAdu []byte) error {
	...
	if sf.crcType != CRCNone {
		pduData = requestAdu[8 : len(requestAdu)-2]
	}
	...
	if sf.crcType != CRCNone {
		binary.BigEndian.PutUint16(responseAdu[4:], uint16(2+len(rspPduData)+2))
	} else {
	...
	fmt.Printf("TX Raw2222 [% x],pdu data lenght:%d\n", rspPduData, tcpHeader.length)
	if sf.crcType != CRCNone {
		checksum := CRC16(responseAdu[6:], sf.crcType)
		if sf.crcEndian {
			responseAdu = append(responseAdu, byte(checksum>>8))
			responseAdu = append(responseAdu, byte(checksum))
		} else {
			responseAdu = append(responseAdu, byte(checksum))
			responseAdu = append(responseAdu, byte(checksum>>8))
		}
