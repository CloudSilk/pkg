...
func Test_protocolFrame_encodeTCPFrame(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, got1, err := tt.this.encodeTCPFrame(tt.args.tid, tt.args.slaveID, tt.args.pdu, CRCNone, true)
...
func TestTCPClientProvider_decodeTCPFrame(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotHead, gotPdu, err := decodeTCPFrame(tt.args.adu, CRCNone, true)
			if (err != nil) != tt.wantErr {
...
func BenchmarkTCPClientProvider_encodeTCPFrame(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_, _, err := tcp.encodeTCPFrame(0, 0, pdu, CRCNone, true)
		if err != nil {
			b.Fatal(err)
		}
	}
...
func BenchmarkTCPClientProvider_decodeTCPFrame(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_, _, err := decodeTCPFrame(adu, CRCNone, true)
		if err != nil {
			b.Fatal(err)
		}
	}
