...
func (sf *protocolFrame) encodeRTUFrame(slaveID byte, pdu ProtocolDataUnit, crc CRCType, crcEndian bool) ([]byte, error) {
	...
	checksum := CRC16(requestAdu, crc)
	if crcEndian {
		requestAdu = append(requestAdu, byte(checksum>>8), byte(checksum))
	} else {
...
func decodeRTUFrame(adu []byte, crcType CRCType, crcEndian bool) (uint8, []byte, error) {
	...
	// Calculate checksum
	crc, expect := CRC16(adu[:len(adu)-2], crcType), binary.LittleEndian.Uint16(adu[len(adu)-2:])
	if crc != expect {
...
func (sf *RTUClientProvider) Send(slaveID byte, request ProtocolDataUnit) (ProtocolDataUnit, error) {
	...
	aduRequest, err := frame.encodeRTUFrame(slaveID, request, sf.crc, sf.crcEndian)
	if err != nil {
		return response, err
	}
	...
	rspSlaveID, pdu, err := decodeRTUFrame(aduResponse, sf.crc, sf.crcEndian)
	if err != nil {
		return response, err
	}
...
func (sf *RTUClientProvider) SendPdu(slaveID byte, pduRequest []byte) ([]byte, error) {
	...
	requestAdu, err := frame.encodeRTUFrame(slaveID, request, sf.crc, sf.crcEndian)
	if err != nil {
		return nil, err
	}
...
func (sf *RTUClientProvider) setCRC(t CRCType, endian bool) {
	sf.crc = t
	sf.crcEndian = endian
}
